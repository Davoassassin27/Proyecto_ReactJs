...
function BotonesCompra({
  ...
  return (
    <section className="botonesDeCompra">
      <p>
        TOTAL CARRITO: $ <span>{total}</span> USD
      </p>
      <button onClick={clear} className="botonVaciar">
        VACIAR CARRITO
      </button>
      <button onClick={() => verifica(datos)} className="botonComprar">
        PROCEDER AL PAGO
        {/* <Link to={"/completado"}> */}
      </button>
      {orden ? (
        <Link onClick={clear} to={`/ticket/${orden} `}>
